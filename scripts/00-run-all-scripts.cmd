...
for /f %%i in (%_PACKER_RUN%) do (
  ...
  if exist "%temp%\%%~ni.err" (
    echo ==^> %%~i returned an error. See "%TEMP%\%%~ni.log" for details.
    echo %date% %time%: %%~i returned an error. See "%TEMP%\%%~ni.log" for details. >>"%_PACKER_LOG%"
    if defined PACKER_PAUSE_ON_ERROR (
      echo       title Press any key to continue . . .
      pause
    )
